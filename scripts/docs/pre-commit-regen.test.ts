...
describe("pre-commit-regen pattern matching", () => {
	it("matches doc.md in hook directories", () => {
		expect(matchesDocPattern("hooks/CodingStandards/TypeStrictness/doc.md")).toBe(true);
	});

	it("matches IDEA.md in hook directories", () => {
		expect(matchesDocPattern("hooks/DuplicationDetection/DuplicationChecker/IDEA.md")).toBe(true);
	});
	...
	it("rejects contract files", () => {
		expect(matchesDocPattern("hooks/CodingStandards/TypeStrictness/TypeStrictness.contract.ts")).toBe(false);
	});

	it("rejects test files", () => {
		expect(matchesDocPattern("hooks/CodingStandards/TypeStrictness/TypeStrictness.test.ts")).toBe(false);
	});
