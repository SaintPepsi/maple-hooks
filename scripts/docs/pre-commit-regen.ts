...
function main(): void {
	...
	// Get staged files
	const result = execSyncSafe("git diff --cached --name-only", { cwd: rootDir });
	if (!result.ok) return;
	...
	process.stderr.write("[pre-commit] Doc sources changed — regenerating HTML docs...\n");

	// Regenerate
	const renderScript = join(import.meta.dir, "render.ts");
	const renderResult = execSyncSafe(`bun run ${renderScript}`, { cwd: rootDir });

	if (!renderResult.ok) {
		process.stderr.write("[pre-commit] Doc regeneration failed (non-blocking)\n");
		return;
	}
