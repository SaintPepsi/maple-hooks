...
| `hook-config.ts`              | Reads hook config from `settings.json` under `hookConfig.{hookName}`. Two overloads: untyped returns `T | null`, typed (with Effect Schema) returns `Result<T, ResultError>` with distinct error codes (`FileReadFailed`, `JsonParseFailed`, `ConfigValidationFailed`). Optional `logStderr` parameter logs failures for debugging |
