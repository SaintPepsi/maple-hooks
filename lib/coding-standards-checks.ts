...
export function isExempted(lines: string[], index: number): boolean {
  ...
  if (current?.includes("@codingstandard-exempt")) return true;
  if (index > 0) {
    const prev = lines[index - 1];
    if (prev?.includes("@codingstandard-exempt")) return true;
  }
  return false;
}
...
export function findInlineImportTypes(lines: string[]): Violation[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const withoutAwaitImports = lines[i].replace(
      /\bawait\s+import\([^)]*\)/g,
      "__RUNTIME_IMPORT__",
    );
...
/** Built-in export-default exclusions — framework conventions that require default exports. */
const DEFAULT_EXPORT_EXCLUSIONS: readonly RegExp[] = [
  /\.config\.(ts|js|mts|mjs)$/, // Config files (vite, tailwind, etc.)
  /spacetimedb\/src\/index\.ts$/, // SpacetimeDB schema
  /\.svelte$/, // Svelte components
  /\/\.storybook\//, // Storybook config files
];

/** Detect export default statements. Named exports only. */
export function findExportDefaults(
  lines: string[],
  filePath?: string,
  additionalExclusions?: readonly RegExp[],
): Violation[] {
  if (filePath) {
    const allExclusions = additionalExclusions
      ? [...DEFAULT_EXPORT_EXCLUSIONS, ...additionalExclusions]
      : DEFAULT_EXPORT_EXCLUSIONS;
...
export interface ViolationCheckOptions {
  exportDefaultExclusions?: readonly RegExp[];
}
...
export function findAllViolations(
  ...
  return [
    ...
    ...findExportDefaults(lines, filePath, options?.exportDefaultExclusions),
  ];
}
...
export function formatViolationSummary(violations: Violation[], filePath: string): string {
  ...
  if (grouped["raw-import"]?.length) {
    const modules = grouped["raw-import"].map(
      (v) => v.content.match(/from\s+["']([^"']+)["']/)?.[1] ?? "unknown",
    );
  ...
  if (grouped["inline-import-type"]?.length) {
    parts.push(
      `${grouped["inline-import-type"].length}x inline import type (use import type declaration)`,
    );
  }
