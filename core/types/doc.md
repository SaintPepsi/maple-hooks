...
- **hook-input-schema.ts** — Effect Schema discriminated union on `hook_event_name`. Provides `parseHookInput(raw)` for validated parsing and `getEventType(input)` for type-safe event resolution. Replaces fragile `"field" in input` / `as Type` casts.
...
- **transcript-schema.ts** — Effect Schema for parsing session transcript JSONL entries. Exports `TranscriptEntrySchema`, `parseTranscriptEntry()`, and types `TranscriptEntry`, `ContentBlock`, `TranscriptMessage`. Shared by RelationshipMemory and RatingCapture hooks for extracting user/assistant messages from session transcripts.
