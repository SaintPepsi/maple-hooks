...
| `log.ts`     | `fs.appendFileSync`                          | `appendHookLog(entry: HookLogEntry)` — structured JSON logging for hook execution. `HookLogEntry` = `{ ts, hook, event, status, duration_ms, session_id?, error?, output_type? }` (8 fields; `output_type` distinguishes "output" vs "silent" hooks). Returns `ok(undefined)` if HOME is unset (fail-open) |
