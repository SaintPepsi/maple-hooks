...
Multi-event contracts (e.g., SteeringRuleInjector handling 7 events) receive different input shapes per event. The runner determines whether the current input is a tool event after parsing, and uses this to decide the safe-exit output format (`{"continue":true}` for tool events, empty for others). The `resolveEvent()` function uses the Effect Schema from `types/hook-input-schema.ts` to discriminate on the `hook_event_name` field — no field-sniffing or type casts. When `parseHookInput` fails and the fallback path is taken, `resolveEvent` logs a warning to stderr via the optional `onFallback` callback.
