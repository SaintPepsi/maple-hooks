...
function makeDeps(
  overrides: Partial<CodingStandardsAdvisorDeps> = {},
...
function unwrap(
  result: Result<ContinueOutput, PaiError>,
): ContinueOutput {
...
describe("CodingStandardsAdvisor", () => {
  ...
  describe("accepts()", () => {
    it("accepts Read on .ts files", () => {
      expect(CodingStandardsAdvisor.accepts(makeReadInput("/src/app.ts"))).toBe(
        true,
      );
    });
    ...
    it("accepts Read on .tsx files", () => {
      expect(
        CodingStandardsAdvisor.accepts(makeReadInput("/src/App.tsx")),
      ).toBe(true);
    });

    it("rejects Edit operations", () => {
      expect(
        CodingStandardsAdvisor.accepts(makeEditInput("/src/app.ts")),
      ).toBe(false);
    });

    it("rejects non-TypeScript files", () => {
      expect(
        CodingStandardsAdvisor.accepts(makeReadInput("/src/style.css")),
      ).toBe(false);
    });
    ...
    it("skips adapter files", () => {
      expect(
        CodingStandardsAdvisor.accepts(
          makeReadInput("/src/adapters/fs.ts"),
        ),
      ).toBe(false);
    });
    ...
    it("skips skipped filenames like vite.config.ts", () => {
      expect(
        CodingStandardsAdvisor.accepts(
          makeReadInput("/project/vite.config.ts"),
        ),
      ).toBe(false);
    });
  });
  ...
  describe("execute()", () => {
    it("continues silently for clean file", () => {
      ...
      const result = unwrap(
        CodingStandardsAdvisor.execute(makeReadInput("/src/math.ts"), deps),
      );
    ...
    it("returns additionalContext for file with raw imports", () => {
      ...
      const result = unwrap(
        CodingStandardsAdvisor.execute(makeReadInput("/src/reader.ts"), deps),
      );
      expect(result.type).toBe("continue");
    ...
    it("returns additionalContext for file with try-catch", () => {
      ...
      const result = unwrap(
        CodingStandardsAdvisor.execute(makeReadInput("/src/handler.ts"), deps),
      );
    ...
    it("returns additionalContext for file with process.env", () => {
      ...
      const result = unwrap(
        CodingStandardsAdvisor.execute(makeReadInput("/src/config.ts"), deps),
      );
    ...
    it("returns additionalContext for file with as any", () => {
      ...
      const result = unwrap(
        CodingStandardsAdvisor.execute(makeReadInput("/src/util.ts"), deps),
      );
    ...
    it("returns additionalContext for file with relative imports", () => {
      ...
      const result = unwrap(
        CodingStandardsAdvisor.execute(makeReadInput("/src/index.ts"), deps),
      );
    ...
    it("continues silently when file cannot be read", () => {
      ...
      const result = unwrap(
        CodingStandardsAdvisor.execute(makeReadInput("/src/missing.ts"), deps),
      );
      expect(result.type).toBe("continue");
      expect(result.additionalContext).toBeUndefined();
    });
    ...
    it("includes violation count in advisory", () => {
      ...
      const result = unwrap(
        CodingStandardsAdvisor.execute(makeReadInput("/src/bad.ts"), deps),
      );
