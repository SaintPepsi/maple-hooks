...
| **DestructiveDeleteGuard** | PreToolUse | Blocks destructive delete patterns in Bash/Edit/Write: `rm -r`, `find -delete`, `python rmtree`, `node/bun rmSync`, `rsync --delete`, `git clean -d`, `ruby rm_rf`, `perl rmtree` (skips .md/.mdx and Dockerfiles). Allows plain `rm` of individual files, including paths with hyphens. |
| **ProtectedBranchGuard** | PreToolUse | Blocks `git commit`, `git push`, `git merge` on protected branches (main/master). Built-in exemptions: `~/.claude` (GitAutoSync), `claude-on-blackberry`. User-configurable exemptions via `settings.json` at `hookConfig.protectedBranchGuard.exemptDirs` (array of directory names). Allows read-only git commands (status, log, diff). Fails open if branch cannot be determined. |
...
| **SpotCheckReview** | Stop | Blocks session end when unpushed files need code review. Spawns Sonnet agent for review. Tracks reviewed file content hashes in `MEMORY/STATE/spot-check/reviewed-hashes.json` to skip files already spot-checked. Escape valve after 1 block marks files as reviewed and releases. Skips `~/.claude` (handled by GitAutoSync). |
