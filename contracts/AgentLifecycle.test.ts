...
import type { SubagentStartInput, SubagentStopInput } from "@hooks/core/types/hook-inputs";
...
describe("AgentLifecycleStart", () => {
  ...
  describe("execute — error handling", () => {
    test("returns ok silent even when ensureDir fails", () => {
      ...
      const result = AgentLifecycleStart.execute(startInput, deps);
      // Should still return ok — lifecycle hooks are best-effort
      expect(result.ok).toBe(true);
      if (result.ok) expect(result.value.type).toBe("silent");
    });
...
describe("AgentLifecycleStop", () => {
  ...
  describe("execute — orphan cleanup", () => {
    ...
    test("does not remove the current agent's own file during cleanup", () => {
      ...
      AgentLifecycleStop.execute(stopInput, deps);
      // The current agent file should NOT be removed during cleanup,
      // even if it looks like an orphan (it was just updated with completedAt)
    ...
    test("cleanup failures are silently ignored", () => {
      ...
      const result = AgentLifecycleStop.execute(stopInput, deps);
      // Should still succeed even if cleanup removal fails
      expect(result.ok).toBe(true);
    });
    ...
    test("skips non-agent files during cleanup", () => {
      ...
      AgentLifecycleStop.execute(stopInput, deps);
      // Should only read agent-*.json files, not other files
