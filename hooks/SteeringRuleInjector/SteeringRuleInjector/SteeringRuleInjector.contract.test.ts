...
describe("SteeringRuleInjector fallback stderr logging", () => {
  it("calls deps.stderr when parseHookInput fails in resolveEvent and getMatchText", () => {
    ...
    expect(stderrMessages.some((m) => m.includes("resolveEvent: schema parse failed"))).toBe(true);
    expect(stderrMessages.some((m) => m.includes("getMatchText: schema parse failed"))).toBe(true);
  });
});
