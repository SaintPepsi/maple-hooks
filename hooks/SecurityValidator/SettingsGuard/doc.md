...
> A Bash command runs `python3 -c "open('settings.json','w')..."`. The command completes, but SettingsRevert detects the change and reverts settings.json to its pre-command state, injecting an error message.
...
- Paired with `SettingsRevert` (PostToolUse) for the revert mechanism
