...
A file only counts as **hook source** if its path contains a `/hooks/` segment *and* matches a watched pattern (`.contract.ts`, `hook.json`, `group.json`, `shared.ts`, `README.md`) *and* is not caught by an `excludePatterns` glob (used to skip directories like `**/fixtures/**`). The `/hooks/` scope guard prevents broad patterns from matching files in unrelated repositories anywhere on disk. `excludePatterns` are matched with `Bun.Glob`, consistent with the Test/DocObligation machines.
...
1. `isHookSourceFile(path, patterns, excludePatterns)` — returns true only if `path` contains `/hooks/` **and** matches a watched pattern **and** is not caught by an exclude glob (e.g. `**/fixtures/**`).
...
export function isHookSourceFile(
  filePath: string,
  patterns: RegExp[],
  excludePatterns: string[] = [],   // globs, e.g. **/fixtures/**
): boolean {
