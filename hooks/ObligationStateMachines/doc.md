...
A file only counts as **hook source** if its path contains a `/hooks/` segment *and* matches a watched pattern (`.contract.ts`, `hook.json`, `group.json`, `shared.ts`, `README.md`). The `/hooks/` scope guard prevents broad patterns from matching files in unrelated repositories anywhere on disk.
...
1. `isHookSourceFile(path, patterns)` — returns true only if `path` contains `/hooks/` **and** matches a watched pattern.
